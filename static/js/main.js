...
async function sendMessage() {
  ...
  // sync hidden fields
  el('response-mode-input').value = responseMode === 'chart' ? chartType : 'table';
  const th = el('topic-hidden');
  if (th) th.value = currentTopic || '';

  // disable button / show dots
  el('btn-text').style.display = 'none';
  el('dots').style.display = 'flex';
  el('send-btn').disabled = true;

  // hide suggestions
  const box = el('suggestions-box');
  if (box) box.style.display = 'none';

  const messages = document.querySelector('.messages');

  // append user message bubble instantly
  const userRow = document.createElement('div');
  userRow.className = 'msg-row user';
  userRow.innerHTML = `
    <div class="msg-label">You</div>
    <div class="bubble">${msgText.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>`;
  messages.appendChild(userRow);

  // append loading dots bubble
  ...
  loadingRow.innerHTML = `
    ...
    <div class="bubble" style="padding:10px 4px">
      <div style="display:flex;gap:5px;align-items:center">
        <span style="width:7px;height:7px;background:#aaa;border-radius:50%;animation:fade 1.2s infinite"></span>
        <span style="width:7px;height:7px;background:#aaa;border-radius:50%;animation:fade 1.2s 0.4s infinite"></span>
        <span style="width:7px;height:7px;background:#aaa;border-radius:50%;animation:fade 1.2s 0.8s infinite"></span>
      </div>
    </div>`;
  ...
  try {
    const res = await fetch('/chat/ui', { method: 'POST', body: formData });
    const html = await res.text();
    const doc = new DOMParser().parseFromString(html, 'text/html');

    // remove loading bubble
    el('loading-bubble')?.remove();

    // get last gemini message from response
    const aiRows = doc.querySelectorAll('.msg-row.gemini');
    const lastAI = aiRows[aiRows.length - 1];
    if (lastAI) {
      messages.appendChild(lastAI);
      wrapTablesWithExport();
      // re-run any inline scripts (charts)
      ...
      // scroll to AI reply start
      lastAI.scrollIntoView({ behavior: 'smooth', block: 'start' });
      lastAISuggestions = extractSuggestionsFromText(lastAI.innerText);
    }

    // update chat_id + URL for new chats
    const newId = doc.querySelector('input[name="chat_id"]')?.value;
    const curInput = form.querySelector('input[name="chat_id"]');
    if (newId && curInput && !curInput.value) {
      curInput.value = newId;
